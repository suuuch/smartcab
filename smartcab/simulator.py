        ...
        if self.log_metrics:
            ...
            if a.learning:
                if self.optimized:  # Whether the user is optimizing the parameters and decay functions
                    if len(add_log_name) > 0:
                        self.log_filename = os.path.join("logs", "sim_improved-learning_%s.csv" % add_log_name)
                        self.table_filename = os.path.join("logs", "sim_improved-learning_%s.txt"% add_log_name)
                    else:
